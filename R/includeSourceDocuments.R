...
#' be uploaded to sites such as RPubs.com or a course management system.
#' 
...
#' 
#' @param \code{fname} character string path and file name of a file to embed.
#' The path should be relative to where the compilation of the Rmd file occurs, 
#' which typically will be in the same directory as the Rmd file.
#' @param \code{...} additional character strings naming files.
#' 
#' @examples
#' \dontrun{
#' # in an R chunk in the Rmd file
#' includeSourceDocuments("sourceFile.Rmd")
#' includeSourceDocuments("sourceFile.Rmd","myData.csv","mySpreadsheet.xlsx")
#' }
#' @export


includeSourceDocuments <- function(fname,...) { 
  ...
  attachFile <- function(file){
    ...
    cat(sprintf(linkHTML, encodedFile))
  }
  # Do the work.
  files <- unlist(list(fname,...)) # change arguments to a vector of strings
  for(F in files) { 
